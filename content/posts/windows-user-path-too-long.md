---
title: "我的環境變數 BANG 不見了"
description: "大會報告，大會報告。現場有一位長度四千多字元的 User PATH 小朋友與 User 走散了！聽到廣播後，請立即前往 System Properties 認領。謝謝。"
slug: "windows-user-path-too-long"
date: 2026-09-18 17:00:00
updated: 2026-09-18 17:00:00
taxonomies:
  tags: [zh_TW, Windows, Terminal, Environment Variable, Computer Concept, WinGet, Symbolic Link]
---
...
>
> 大會報告，大會報告。
> 現場有一位長度四千多字元的 User PATH 小朋友與 User 走散了！
> 聽到廣播後，請立即前往 System Properties 認領。謝謝。
...
[^1]: Microsoft 曾記錄過幾乎相同的 Shell32 問題：收到 `WM_SETTINGCHANGE` 後重新建立 environment block 時，過長的 `PATH` 會令枚舉提早失敗，留下部分初始化的 environment block。參見：[Microsoft Support](https://support.microsoft.com/en-us/servicing/os/windows/2017/01/error-messages-after-you-change-the-2047-character-limit-in-an-environment-variable-on-a-computer-th)。現代 Windows 11 上亦有 WinGet 使用者回報 User PATH 超過 2047 字元後，CMD 無法取得正確環境，進而影響 VS Code：[microsoft/winget-cli#5557](https://github.com/microsoft/winget-cli/issues/5557)。
...
最長的莫過於 [WinGet](https://github.com/microsoft/winget-cli) 所安裝程式的路徑，例如：
...
但是 Windows 在一般權限下建立 symbolic link 有額外限制，如果沒有開啟「[開發者模式（Developer Mode）](ms-settings:developers)」，非提升權限的程式可能無法建立符號連結。
...
[^2]: WinGet portable package 在無法建立 symbolic link 的情況下會改走 PATH-based installation；相關行為可見 [microsoft/winget-cli#2909](https://github.com/microsoft/winget-cli/issues/2909)、[#5220](https://github.com/microsoft/winget-cli/issues/5220)。大量 package directory 導致 PATH 膨脹的問題另見 [#3601](https://github.com/microsoft/winget-cli/issues/3601)；卸載後 PATH 殘留則另有 [#6160](https://github.com/microsoft/winget-cli/issues/6160)。
...
- 若是 WinGet 使用者，在安裝 portable package 前建議先啟用「[開發者模式](ms-settings:developers)」。
